...
static INLINE void sw_spi_data_size( uint32_t dataSizeBits ) {
	...
	uint32_t tmp = spi->SPI->CR2;
	tmp &= ~SPI_CR2_DS;										// Reset DS value
	tmp |= ((uint32_t)dataSizeBits - 1) << SPI_CR2_DS_Pos;	// Set data size
	spi->SPI->CR2 = tmp;
...
#ifdef SOFT_NSS
	spi->SPI->CR1 |= SPI_CR1_SSM 					// Software slave management
				  |  SPI_CR1_SSI; 					// Internal slave select
	gpio_pin_cfg( SOFT_NSS_PORT, SOFT_NSS_PIN, gpio_mode_output_PP_HS );	// SPI NSS SW management
	NSS_HI();
#else
	spi->SPI->CR2 |= SPI_CR2_SSOE;	// SS Output Enable
	spi->SPI->CR2 |= SPI_CR2_NSSP; 	// NSS pulse management Enable
...
//		RCC->APB1ENR |= RCC_APB1ENR_SPI2EN;
	...
	gpio_pin_cfg( spi->sck_port,  spi->sck_pin,  spi->alternateFun );	// SPI CLK
	gpio_pin_cfg( spi->miso_port, spi->miso_pin, spi->alternateFun );	// SPI MISO
	gpio_pin_cfg( spi->mosi_port, spi->mosi_pin, spi->alternateFun );	// SPI MOSI
// Default 8-bit data
	sw_spi_nss_mode();
#ifdef SPI_DMA
	spi_dma_init();
...
/* 10. *******************************************************************************/
uint8_t sw_spi_write_read( uint16_t data, uint32_t bitSize ) {
	sw_spi_data_size( bitSize );
	uint16_t dataLoc = 0;
#ifdef SOFT_NSS
	NSS_LO();
